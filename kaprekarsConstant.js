...
function KaprekarsConstant(num) { 
  ...
  while (numCopy !== 6174){
    let sortedAsc = parseInt(numCopy.toString().split('').sort().join(''));
    let sortedDec = parseInt(numCopy.toString().split('').sort((a, b) => b - a).join(''));
    
    if (sortedDec.toString().length < 4) {
      for (let i = sortedDec.toString().length; i < 4; i++){
        sortedDec = parseInt(sortedDec.toString() + '0');
      }
    }

    numCopy = sortedDec - sortedAsc;
    count += 1;
  }
